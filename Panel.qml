...
Panel {
  ...
  readonly property var session: sessionWindow(provider)
  ...
  function sessionWindow(p) {
    ...
    for (var i = 0; i < windows.length; i++) {
      if (windowSpanMs(windows[i].title) === 5 * 3600 * 1000
          || String(windows[i].title).toLowerCase().indexOf("session") >= 0)
        return windows[i]
    }
  ...
  function weeklyWindow(p) {
    var windows = limitWindows(p)
    for (var i = 0; i < windows.length; i++) {
      if (windowIsLong(windows[i].title)) return windows[i]
    }
    return null
  }
  ...
  function absoluteReset(resetAt, includeDate) {
    if (!resetAt) return "—"
        ...
        Column {
          ...
          Column {
            ...
            Text {
              width: parent.width
              text: root.session
                ? "5h: " + Math.round(root.session.percent * 100) + "% used"
                  + " · reset " + root.absoluteReset(root.session.resetAt, false)
                : "5h: —"
              color: root.foreground
              font.family: root.fontFamily
              font.pixelSize: Style.font.body
            ...
            Text {
              width: parent.width
              text: root.weekly
                ? "Week: " + Math.round((1 - root.weekly.percent) * 100) + "% left"
                  + " · reset " + root.absoluteReset(root.weekly.resetAt, true)
                : "Week: —"
              color: root.foreground
              font.family: root.fontFamily
              font.pixelSize: Style.font.body
            ...
            Text {
              width: parent.width
              text: "Tokens: " + usage.formatTokenCount(root.totalTokens(root.provider))
              color: root.dim
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
            }
  ...
  component LimitRow: Column {
    ...
    Text {
      ...
      text: {
        ...
        return limitRow.window
          ? "Resets " + root.absoluteReset(limitRow.window.resetAt,
              String(limitRow.window.title).toLowerCase().indexOf("week") >= 0)
          : ""
      }
      color: root.dim
      font.family: root.fontFamily
